...
def question():
    ...
    questions_cur = db.execute('select question_text, asked_by_id from questions')
...
def ask():
    ...
    if request.method == "POST":
        db.execute('insert into questions(question_text, asked_by_id, expert_id) values(?, ?, ?)', [request.form['question'], request.form['expert'], request.form['expert']]) 
        db.commit()
        return redirect(url_for('index'))
