...
def question(question_id):
    ...
    questions_cur = db.execute('select questions.question_text, questions.answer_text, askers.name as asker_name, experts.name as expert_name from questions join \
        users as askers on askers.id = questions.asked_by_id join users as experts on experts.id = questions.expert_id where questions.id = ?', [question_id])
    questions_result = questions_cur.fetchone()


    return render_template('question.html', user=user, questions=questions_result)
...
def ask():
    ...
    return render_template('ask.html', user=user, expert=expert_results)
